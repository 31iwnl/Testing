...
INPUT_CSV = config.get('input_csv', 'isd-history.csv')
...
def get_ftp_file_mdtm(ftp, filepath):
    try:
        ...
        logging.warning(f"Failed to get MDTM for {filepath}: {e}")
    return None


def download_isd_history_csv(output_file):
    try:
        ...
        if remote_mdtm is None:
            logging.warning("Could not get remote file modification time, downloading anyway")
        else:
        ...
        logging.error(f"Failed to download ISD history CSV: {e}")
        return False
